-- CORREÇÃO CRÍTICA: Corrigir inconsistência e melhorar trigger

-- 1. Primeiro, corrigir os residentes existentes que foram afetados pela alteração do código
UPDATE residents 
SET condominium_id = (
  SELECT c.id 
  FROM condominiums c 
  WHERE c.resident_linking_code = (
    SELECT u.raw_user_meta_data->>'linking_code'
    FROM auth.users u 
    JOIN profiles p ON u.id = p.user_id 
    WHERE p.id = residents.profile_id
  )
)
WHERE id IN (
  SELECT r.id 
  ...
  WHERE u.raw_user_meta_data->>'linking_code' != c.resident_linking_code
);

-- 2. Corrigir perfis também
UPDATE profiles 
SET condominium_id = (
  SELECT c.id 
  FROM condominiums c 
...
WHERE id IN (
  ...
  FROM profiles p
  JOIN auth.users u ON p.user_id = u.id
  JOIN condominiums c ON p.condominium_id = c.id
  WHERE u.raw_user_meta_data->>'linking_code' != c.resident_linking_code
  AND p.role = 'resident'
);

-- 3. Adicionar validação adicional ao trigger para evitar problemas futuros
