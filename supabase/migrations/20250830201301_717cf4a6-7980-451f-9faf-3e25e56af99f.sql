...
-- Fase 1: Corrigir códigos existentes primeiro
-- =====================================================

-- 1. Verificar e corrigir códigos existentes antes de aplicar constraint
-- Primeiro, vamos ver quais códigos não estão no formato correto
DO $$
DECLARE
    _condo RECORD;
    _new_code text;
BEGIN
    FOR _condo IN 
        ...
        FROM condominiums 
        WHERE NOT (resident_linking_code ~ '^[a-f0-9]{16}$')
    LOOP
        -- Gerar novo código válido
        _new_code := encode(extensions.gen_random_bytes(8), 'hex');
        
        -- Garantir que é único
        WHILE EXISTS (SELECT 1 FROM condominiums WHERE resident_linking_code = _new_code) LOOP
            _new_code := encode(extensions.gen_random_bytes(8), 'hex');
        END LOOP;
        ...
        RAISE LOG 'FIXED_LINKING_CODE: condominium=% (%), old_code=%, new_code=%', 
            _condo.name, _condo.id, _condo.resident_linking_code, _new_code;
    END LOOP;
END
$$;

-- 2. Agora aplicar as funções corrigidas
...
-- 3. Função melhorada de regeneração com validação tripla
...
-- 4. Função de validação super robusta
...
-- 5. Agora aplicar a constraint com segurança
