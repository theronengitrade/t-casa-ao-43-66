-- ============================================================================
-- CORREÇÃO CRÍTICA: RECRIAR SISTEMA COMPLETO DE REGISTRO DE MORADORES
-- ============================================================================

-- 1. RECRIAR A FUNÇÃO handle_new_user com LOGS EXTREMAMENTE DETALHADOS
...
-- 3. VERIFICAR SE DADOS DE TESTE EXISTEM (código fecf7bee03b5f41c)
DO $$
DECLARE
    test_condominium_id UUID;
    apartment_count INTEGER;
BEGIN
    -- Verificar se condomínio de teste existe
    SELECT id INTO test_condominium_id 
    FROM public.condominiums 
    WHERE resident_linking_code = 'fecf7bee03b5f41c';
    
    IF test_condominium_id IS NOT NULL THEN
        -- Verificar se tem apartamentos
        SELECT COUNT(*) INTO apartment_count
        FROM public.apartments 
        WHERE condominium_id = test_condominium_id;
        
        -- Se não tem apartamentos, criar alguns para teste
        IF apartment_count = 0 THEN
            INSERT INTO public.apartments (condominium_id, number, floor, bedrooms, bathrooms, is_occupied)
            VALUES 
                (test_condominium_id, '102', '1', 2, 1, false),
                (test_condominium_id, '103', '1', 2, 1, false),
                (test_condominium_id, '104', '1', 3, 2, false),
                (test_condominium_id, '202', '2', 2, 1, false),
                (test_condominium_id, '203', '2', 2, 1, false);
            
            RAISE LOG 'TEST_DATA: Created 5 test apartments for condominium %', test_condominium_id;
        ELSE
            RAISE LOG 'TEST_DATA: Condominium % already has % apartments', test_condominium_id, apartment_count;
        END IF;
    ELSE
        RAISE LOG 'TEST_DATA: Test condominium with code fecf7bee03b5f41c not found';
    END IF;
END $$;
...
DO $$
BEGIN
    RAISE LOG 'REGISTRATION_SYSTEM_DEPLOYED: Trigger and function recreated successfully at %', now();
END $$;